...
int rc_read_dictionary (rc_handle *rh, const char *filename)
{
	...
	{
		// rc_log(LOG_ERR, "rc_read_dictionary: couldn't open dictionary %s: %s",				filename, strerror(errno));
		return -1;
	}
	...
	while (fgets (buffer, sizeof (buffer), dictfd) != NULL)
	{
		...
		if (strncmp (buffer, "ATTRIBUTE", 9) == 0)
		{
			...
			{
				// rc_log(LOG_ERR, "rc_read_dictionary: invalid attribute on line %d of dictionary %s",		line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			{
				// rc_log(LOG_ERR, "rc_read_dictionary: invalid name length on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			if (!isdigit (*valstr))
			{
				// rc_log(LOG_ERR,				 "rc_read_dictionary: invalid value on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			value = atoi (valstr);
			...
			{
				// rc_log(LOG_ERR,				  "rc_read_dictionary: invalid type on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			if (optstr[0] != '\0') {
				...
				for (cp1 = optstr; cp1 != NULL; cp1 = cp) {
					...
					if (dvend == NULL) {
						// rc_log(LOG_ERR,						 "rc_read_dictionary: unknown Vendor-Id %s on line %d of dictionary %s",							 cp1, line_no, filename);
						fclose(dictfd);
						return -1;
					}
				}
		...
		else if (strncmp (buffer, "VALUE", 5) == 0)
		{
			...
			if (sscanf (buffer, "%s%s%s%s", dummystr, attrstr,
				    namestr, valstr) != 4)
			{
				// rc_log(LOG_ERR,			   "rc_read_dictionary: invalid value entry on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			{
				// rc_log(LOG_ERR,		      "rc_read_dictionary: invalid attribute length on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			{
				// rc_log(LOG_ERR,				 "rc_read_dictionary: invalid include entry on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			ifilename = namestr;
		...
		else if (strncmp (buffer, "VENDOR", 6) == 0)
		{
			...
			{
				// rc_log(LOG_ERR,				 "rc_read_dictionary: invalid Vendor-Id on line %d of dictionary %s",					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
